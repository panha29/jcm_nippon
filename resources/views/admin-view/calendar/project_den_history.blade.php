...
@php
    $html_tag_data = [];
    $title = 'Project Den History';
    $description= 'Ecommerce Project Den History'
    @endphp
        ...
        <table id="project_table_export" class="stripe" style="width:100%">
            <thead hidden>
                <tr>
                    ...
                    <th class="project-den-th">ជាងដិន</th>
                    <th class="project-den-th">ថ្ងៃចាប់ផ្តើមដិន</th>
                    <th class="project-den-th">ថ្ងៃបញ្ចប់ដិន</th>
                    ...
                    <tr >
                        ...
                        <td class="pjlabelkh pjlabelkh-height">
                            <div class="scrollbar" id="style-1">
                                <div class="force-overflow" style="text-align: center">{{$item->den_tailor}}</div>
                            </div>
                        </td>
                        <td class="pjlabelkh pjlabelkh-height">{{$item->den_start_date}}</td>
                        <td class="pjlabelkh pjlabelkh-height">{{$item->den_end_date}}</td>
                        <td class="pjlabelkh pjlabelkh-height">{{$item->pj_customer_name}}</td>
                        <td class="pjlabelkh pjlabelkh-height">{{$item->project}}</td>
                        <td style="text-align: center">{{$item->quantity}}</td>
                        <td>
                            <a data-bs-toggle="modal" data-bs-target="#pjden{{$item->id}}">
                                <label for="progress_den"><progress max="100" value="{{$item->progress_den}}" style="--p:{{$item->progress_den}}%">{{$item->progress_den}}%</progress></label>
                            </a>
                        </td>
                    </tr>
                @endforeach
                ...
                <table id="project_table" class="stripe" style="width:100%">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="project-den-th">Project</th>
                            <th class="project-den-th">ចំនួន</th>
                            {{-- <th class="project-den-th">ថ្ងៃកម្ម៉ង</th>
                            <th class="project-den-th">ថ្ងៃកាត់</th>
                            <th class="project-den-th">ថ្ងៃមកល</th>
                            <th class="project-den-th">ថ្ងៃមកយក</th>
                            <th class="project-den-th">ថ្ងៃបញ្ចប់</th> --}}
                            {{-- <th class="project-den-th">ថ្ងៃពន្យាពេល</th>
                            <th class="project-den-th">បញ្ហាផ្សេងៗ</th> --}}
                            <th class="project-den-th">Progress</th>
                    ...
                    <tbody id="tbody">
                        ...
                        @endphp
                            <tr style="background-color: {{$color}}" class="hidetr">
                                ...
                                <td class="pjlabelkh pjlabelkh-height">{{$item->project}}</td>
                                <td style="text-align: center">{{$item->quantity}}</td>
                                {{-- <td style="text-align: center">{{$item->order_date}} <br> {{$item->order_time}}</td>
                                <td style="text-align: center">{{$item->process_start_date}} <br> {{$item->process_start_time}} </td>
                                <td style="text-align: center">{{$item->testing_date}} <br> {{$item->testing_time}}</td>
                                <td style="text-align: center">{{$item->pickup_date}} <br> {{$item->pickup_time}} </td>
                                <td style="text-align: center">{{$item->start_date}}</td>
                                <td style="text-align: center">{{$item->end_date}}</td> --}}
                                {{-- <td style="text-align: center">{{$item->delay_date}}</td>
                                <td style="text-align: center">{{$item->problem}}</td> --}}
                                <td>
                                    <a data-bs-toggle="modal" data-bs-target="#pjden{{$item->id}}">
                                        <label for="progress_den"> {{$item->progress_den}}% <progress max="100" value="{{$item->progress_den}}" style="--p:{{$item->progress_den}}%">{{$item->progress_den}}%</progress></label>
                                    </a>
                                    @include('admin-view.calendar.project_den_progress')
                                </td>
                                <td>
